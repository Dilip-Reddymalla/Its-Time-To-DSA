  ...
  return (
    <section className="section" style={{ padding: '0 24px 100px' }}>
      <div className="container relative overflow-hidden" style={{ borderRadius: '48px', padding: '80px 24px', background: 'linear-gradient(135deg, #4f46e5 0%, #312e81 100%)', boxShadow: '0 40px 100px -20px rgba(79, 70, 229, 0.4)' }}>
        ...
        <div style={{ position: 'absolute', bottom: '-20%', left: '-5%', width: '30%', height: '100%', background: 'radial-gradient(ellipse at center, rgba(255,255,255,0.05) 0%, transparent 70%)', zIndex: 1, filter: 'blur(30px)' }}></div>
        ...
        <div className="reveal visible" style={{ position: 'relative', zIndex: 10, display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center' }}>
          <h2 style={{ fontSize: 'clamp(2.5rem, 6vw, 4rem)', fontWeight: '900', color: 'white', marginBottom: '24px', letterSpacing: '-0.04em', lineHeight: '1.1' }}>
            Ready to <span style={{ color: 'var(--emerald-400)' }}>conquer</span> your grind?
          </h2>
          
          <p style={{ color: 'rgba(255,255,255,0.7)', fontSize: '1.25rem', marginBottom: '48px', maxWidth: '650px', lineHeight: '1.6', fontWeight: '500' }}>
            Join 10,000+ developers leveraging structured patterns and automated tracking to crush their technical interviews.
          </p>

          <Link to="/login" className="btn btn-primary" style={{ background: 'white', color: '#4f46e5', padding: '20px 48px', fontSize: '1.25rem', fontWeight: '800', borderRadius: '24px', boxShadow: '0 20px 50px rgba(0,0,0,0.3)', transition: 'transform 0.3s cubic-bezier(0.34, 1.56, 0.64, 1)', display: 'flex', alignItems: 'center', gap: '12px' }} onMouseOver={e => e.currentTarget.style.transform = 'scale(1.05) translateY(-4px)'} onMouseOut={e => e.currentTarget.style.transform = 'scale(1) translateY(0)'}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: '32px', marginTop: '56px', color: 'rgba(255,255,255,0.6)', fontSize: '1rem', fontWeight: '600' }}>
